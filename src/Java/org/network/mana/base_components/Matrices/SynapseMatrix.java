...
public class SynapseMatrix {
    ...
    public final SpikingNeuron src;

    public final LIFNeurons tar;
  ...
  //  public MANAMatrix(int offsetSrc, int offsetTar, COOManaMat cooMat, Neuron src, MANANeurons tar) {
    public SynapseMatrix(COOManaMat cooMat, SpikingNeuron src, LIFNeurons tar) {
        ...
        weightsTOrd = new InterleavedSparseMatrix(cooMat.data, targRange, noSrc, noTar,
                //offsetTar, offsetSrc,
                Ordering.TARGET);
       ...
       // pfrBuffers = new InterleavedSparseAddOn(weightsTOrd, 1);
        ...
        outDataSOrd = new InterleavedSparseMatrix(cooMat.data, srcRange, noTar, noSrc,
                //offsetSrc, offsetTar,
                Ordering.SOURCE);
     ...
     * @param maxDly
     */
    public SynapseMatrix(SpikingNeuron src, LIFNeurons tar,
...
//    }
    ...
    public void clip(double ceil, double floor) {
        for(int ii=0; ii<weightsTOrd.getRawData().length; ii+=2) {
            if(weightsTOrd.getRawData()[ii] > ceil) weightsTOrd.getRawData()[ii] = ceil;
        }
        for(int ii=0; ii<weightsTOrd.getRawData().length; ii+=2) {
            if(weightsTOrd.getRawData()[ii] < floor) weightsTOrd.getRawData()[ii] = floor;
        }
    }
    ...
    public static void main(String [] args) {
        ...
        SynapseMatrix mm = new SynapseMatrix(src, tar,
        ...
        SynapseMatrix convMana = new SynapseMatrix(coomana, src, tar);
