...
public class MANA_Sector implements Syncable {
    ...
    public final Map<SpikingNeuron, MANA_Node> childNodes = new TreeMap<SpikingNeuron, MANA_Node>(
            (SpikingNeuron a, SpikingNeuron b) -> {
                if (a==b) {
                    return 0;
                }
                if (a.getID() < b.getID()) {
    ...
    public void init() {
        ...
        for(MANA_Node node : childNodes.values()) {
            if(node.srcData.isExcitatory()&& !(node.srcData instanceof MANANeurons)) {
     ...
     * @param specs
     */
    public MANA_Node add(SpikingNeuron src, ConnectSpecs specs) {
     ...
     * @param specs
     */
    public MANA_Node add(SpikingNeuron src, ConnectSpecs specs, DampFunction damp, STDP rule) {
        MANA_Node newEntry = MANA_Node.buildNodeAndConnections(
