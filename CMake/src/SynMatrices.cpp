...
SynMatrices* SynMatrices::connectNeurons(	GenericNeuron &_src,
											...
											const bool useUDF )
{
	assert(&_src.host == &_tar.host); //no trans-network synapses

	SynMatrices* syns = new SynMatrices(_src, _tar, _splas, _maxDly, _minDly);
	if (_src.pol)
	{
...
uint32_t* SynMatrices::calcDelayMat(	const GenericNeuron* _src,
...
void SynMatrices::propagate()
{
	...
	array postTriggered = tarHost.spks((*ijInds).col(1));
	...
	if (srcPol) {
		tarHost.I_e += results(where(diff1(inds))); // where(diff()) shows last position of each target neuron i.e. where the scan operation will have put the sum over all going to that j/neuron
	} else {
		tarHost.I_i += results(where(diff1(inds)));
	}
	...
	array pdws = dt*(tarHost.lastSpkTimes-_time);
	splas->preTrigger(pdws); 

	(*wt_And_dw)(arrivals,1) = pdws(inds);

	pdws = dt*(srcHost.lastSpkTimes-_time);
	splas->postTriggered(pdws);

	(*wt_And_dw)(postTriggered, 1) = pdws((*ijInds)(postTriggered, 0));
	 ...
	 minDly(_minDly), maxDly(_maxDly), usingUDF(true)
{
	...
	uint32_t* dlys = calcDelayMat(_src, _tar, _maxDly, _minDly);
	...
	*srcDlyInds = Utils::srcInd2dlysrcInd(_src->size, (*ijInds)(span,0), *dlyArr);
	...
	tarStartFin = new array(Utils::findStAndInDegs(ijInds));
