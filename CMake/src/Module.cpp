...
Module::Module(const Network& _netHost, const uint32_t _size, const Position _minPos, const Position _maxPos)
    : Module(_netHost, _size, _minPos, _maxPos, DEF_IE_RATIO)
{
}

MANA_Module::MANA_Module(const Network& _netHost, const uint32_t _size, const Position _minPos, const Position _maxPos)
...
{
    excNeuGrp = new AIFNeuron(_netHost, (uint32_t)(size * (1 - _ieRatio)), Polarity::EXC, minPos, maxPos);
    inhNeuGrp = new AIFNeuron(_netHost, (uint32_t)(size * _ieRatio), Polarity::INH, minPos, maxPos);
    hpExc = NULL;
    hpInh = NULL;
    ipExc = NULL;
    ipInh = NULL;
                                           ...
                                           const float _ieRatio)
{
    ...
    MANA_Module* mod = new MANA_Module(_netHost, _size, _minPos, _maxPos, _ieRatio);
    ...
    SynMatrices* eeSyns = SynMatrices::connectNeurons(
        *(mod->excNeuGrp), *(mod->excNeuGrp), eeSTDP, minDelay, maxDelay, MANA_Module::DEF_INIT_DENSE, true);
    SynMatrices* eiSyns = SynMatrices::connectNeurons(
        *(mod->excNeuGrp), *(mod->inhNeuGrp), eiSTDP, minDelay, maxDelay, MANA_Module::DEF_INIT_DENSE, true);
    SynMatrices* ieSyns = SynMatrices::connectNeurons(
        *(mod->inhNeuGrp), *(mod->excNeuGrp), ieSTDP, minDelay, maxDelay, MANA_Module::DEF_INIT_DENSE, true);
    SynMatrices* iiSyns = SynMatrices::connectNeurons(
        *(mod->inhNeuGrp), *(mod->inhNeuGrp), iiSTDP, minDelay, maxDelay, MANA_Module::DEF_INIT_DENSE, true);
...
void MANA_Module::iterate_one()
{
    ...
    std::cout<< "calc'd syn propagations" << '\n';
    sNrmExc->perform(*(ipExc->prefFR), lambda);
    std::cout<< "calc'd syn nrm" << '\n';
    excNeuGrp->runForward();
    std::cout<< "calc'd neuron acts" << '\n';
    hpExc->perform(*(ipExc->prefFR));
    std::cout<< "calc'd new thresh" << '\n';
    ipExc->perform();
    std::cout<< "calc'd new tar frs" << '\n';
