...
/**
 * MANA nodes are comprised of a set of source neurons (either input neurons or other MANA neurons) and target neurons
 * and contain all the weights and other relevant synapse data for the connections between that source and target. MANA
 * nodes are the work-horse and each one comprises a separate update task to be sent to a worker thread. They are
